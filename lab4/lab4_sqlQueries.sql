...
FROM employees
GROUP BY department ORDER BY average_salary DESC;
...
SELECT e.employee_id, CONCAT(e.first_name,' ', e.last_name) AS full_name,e.department
FROM employees e
...
--Task 7.1
SELECT
    CONCAT(e.first_name,' ', e.last_name) AS full_name, e.department, e.salary,
...
ORDER BY e.department, salary_rank, full_name;

--Task 7.2
SELECT p.project_name, SUM(a.hours_worked) AS total_hours, COUNT(DISTINCT a.employee_id) AS number_of_employees
FROM projects p
JOIN assignments a ON p.project_id = a.project_id
GROUP BY p.project_id, p.project_name
...
SELECT
    department,
    LEAST(COUNT(*), 10) AS total_employees,
    GREATEST(ROUND(AVG(salary), 2), 40000) AS average_salary,
